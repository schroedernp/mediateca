<?php
        ...
        //código upload foto
        $foto=basename($_FILES["ficheiro2"]["name"]);
        $target_dir = "capa/";
        $target_file = $target_dir . basename($_FILES["ficheiro2"]["name"]);
        $uploadOk = 1;
        $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

        // verifica se é uma imagem
        if(isset($_POST["submit"])) {
            $check = getimagesize($_FILES["ficheiro2"]["tmp_name"]);
            if($check !== false) {
        ...
        if ($_FILES["ficheiro2"]["size"] > 5000000) { //5MB
